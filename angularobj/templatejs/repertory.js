my.controller('repertoryController', ['$scope','$http',function($scope,$http,$route) {
	$scope.$route = $route;
	//页面初始化
	showPage(1);
	//时间戳转换
	function time(timer){
		return  new Date(timer).toLocaleString().replace('/', "-").replace('/', "-").slice(0,16);
	}
    function showPage(pageIndex){
    	jQuery.ajax({
	    	type:"get",
	    	url:url+"input/getList.do?pageNum="+pageIndex,
	    	async:true, 
	    	xhrFields: {
	    	...
	    	success:function(data){
				...
				for (var i=0 ;i<data.data.list.length;i++) {
	    			str += '<tr  data-id="'+data.data.list[i].providerId+'" >'+
						'<td width="30px"><input data-id="'+data.data.list[i].inputId+'" type="checkbox" name="name" value=""/></td>'+
						'<td nowrap="nowrap"><a class="datauid">'+Number(i+1)+'</a></td>'+
//						'<td nowrap="nowrap">'+data.data[i].userName+'</td>'+
						'<td>'+data.data.list[i].inputNum+'</td>'+
						'<td>'+data.data.list[i].provider+'</td>'+
						'<td>'+data.data.list[i].classify+'</td>'+
						'<td>'+data.data.list[i].inputCount+'</td>'+
						'<td nowrap="nowrap">'+time(data.data.list[i].inputTime)+'</td>'
...
//  alert(arr.unique());
	...
	jQuery("#editer").on("click",function(){
		jQuery.ajax({
			...
			success:function(msg){
				console.log(msg);
				var strr = '<option value="">--请选择--</option>';
				for (var i=0;i<msg.data.length;i++) {
					strr += '<option data-id="'+msg.data[i].providerType+'" value="'+msg.data[i].providerId+'">'+msg.data[i].providerName+'</option>'
				}
				jQuery('.providerType').html(strr)
			}
		});
		jQuery(".cower").show();
		jQuery(".tipBox").show();	
	...
	//分类
	jQuery('.providerType').on('change',function(){
...
//			
			jQuery.ajax({
				...
				data:{					
					...
					providerId:jQuery('.providerType option:selected').val(),
					provider:jQuery('.providerType option:selected').text(),
					classify:jQuery('.fenlei').val()					
				},
				dataType:"json",
				success:function(data){
					console.log(data)
