...
GridTree.prototype.makeTable=function(){if(!elct){console.log('����:Ҫ��Ⱦdiv������!');return false;}
...
var columnName=tc['idColumn'];if(typeof data[0][columnName]=='undefined'){_isValid=false;console.log("���õ�����[idColumn]ֵ����,����!");return;}
var columnName=tc['parentColumn'];if(typeof data[0][columnName]=='undefined'){_isValid=false;console.log("���õ�����[parentColumn]ֵ����,����!");return;}
GridTree.prototype._makeTable(tableTree,data);}catch(e){GridTree.prototype._makeTableWithNoData(tableTree);}}else{elct.innerHTML="û�����.";}}});}else{jQuery.ajax({type:"POST",url:tc.dataUrl,async:true,data:['idColumn=',tc.idColumn,"&parentColumn=",tc.parentColumn,"&gtlimit=",tc.pageSize].join(''),success:function(msg){if(msg!=null&&msg!=""){try{eval(" tableDataArr="+msg);var data=tableDataArr.data;var columnName=tc['idColumn'];if(typeof data[0][columnName]=='undefined'){_isValid=false;console.log("���õ�����[idColumn]ֵ����,����!");return;}
var columnName=tc['parentColumn'];if(typeof data[0][columnName]=='undefined'){_isValid=false;console.log("���õ�����[parentColumn]ֵ����,����!");return;}
GridTree.prototype._makeTable(tableTree,msg);}catch(e){GridTree.prototype._makeTableWithNoData(tableTree);}}else{elct.innerHTML="û�����.";}}});}}
...
showTypeDesc.onclick();};for(var i=0;i<allvalues.length;i++){if(findInArray(chArry,allvalues[i])!=-1){createCheckbox(node,setDisabled,'',inputName,allvalues[i],f,texts[i],cssName,'checked');}else{createCheckbox(node,setDisabled,'',inputName,allvalues[i],f,texts[i],cssName,'');}}}else{node.innerHTML='<font color="red">���ñ�����������ͳ���.</font>';}}
return node;}
...
{divNode=_$("_msgDiv");var table=_$('_trhead');if(v){divNode.innerText='�����ύ...';divNode.style.top=table.offsetTop+10;divNode.style.left=table.offsetLeft+table.offsetWidth/9*8;divNode.style.display='inline';}
else
divNode.style.display='none';}
GridTree.prototype._createMsgDiv=function(){msgDiv=document.createElement("div");msgDiv.setAttribute('id','_msgDiv');msgDiv.className='msgdiv';msgDiv.appendChild(document.createTextNode("�����ύ..."));document.body.appendChild(msgDiv);}
...
if(tc.rowCount==true&&tc.countColumnDesc==null){tc.countColumnDesc='����';}
if(!(tc.checkOption+''=="1"&&tc.checkOption+''=="2")&&tc.checkColumnDesc==null){tc.checkColumnDesc='��ѡ��';}
...
GridTree.prototype._checkData=function(obj,dataColumn){if(_isValid){if(typeof obj[dataColumn]=='undefined'){_isValid=false;console.log("�����ñ��������["+dataColumn+"]!");return;}
if(typeof tc.data!='undefined'){if(dataColumn=='idColumn')
{var columnName=obj['idColumn'];if(typeof tc.data[0][columnName]=='undefined'){_isValid=false;console.log("���õ�����[idColumn]ֵ����,����!");return;}}
else if(dataColumn=='parentColumn')
{var columnName=obj['parentColumn'];if(typeof tc.data[0][columnName]=='undefined'){_isValid=false;console.log("���õ�����[parentColumn]ֵ����,����!");return;}}}
_isValid=true;}}
...
this._resetPageBtns();var gotime=new Date()-o;this._wirteDebug('��һ����ʾǰ̨���ʱ��:'+gotime);if(tc.disabled)
this.setDisabled(1);}
...
var gotime=new Date()-o;this._wirteDebug('ǰ̨���з������ʱ��:'+gotime);}
...
GridTree.prototype._serverAnalyseData=function(msg){var o=new Date();eval("tc.allDataInfoWithPageInfo="+msg);parents=tc.allDataInfoWithPageInfo.parents;firstLevelNodes=tc.allDataInfoWithPageInfo.firstLevelNodes;parentToChildMap=jsonMapToJsHashMap(tc.allDataInfoWithPageInfo.parentToChildMap);nodeMap=jsonMapToJsHashMap(tc.allDataInfoWithPageInfo.idToNodeMap);childToFatherMap=jsonMapToJsHashMap(tc.allDataInfoWithPageInfo.idToParent);var gotime=new Date()-o;this._wirteDebug('ǰ̨������̨��json����ʱ:'+gotime);}
...
GridTree.prototype._makePageBar=function(tableObj,index){var pageBar=document.createElement('table');pageBar.setAttribute('id',tc.pageBarId);pageBar.cellPadding='0px';pageBar.cellSpacing='0px';var pageBarTR=pageBar.insertRow(0);var firstCell=pageBarTR.insertCell(pageBarTR.cells.length);firstCell.setAttribute('id','firstPageCell');firstCell.innerHTML='<button onclick="GridTree.prototype._toFirstPage(event)" class="pagebarbtn disFirstPage" title="��һҳ" id="_firstPageBtn" disabled></button>';var preCell=pageBarTR.insertCell(pageBarTR.cells.length);preCell.setAttribute('id','prePageCell');preCell.innerHTML='<button onclick="GridTree.prototype._toPrePage()" class="pagebarbtn disPrevPage" title="ǰһҳ" id="_prePageBtn" disabled></button>';var afterCell=pageBarTR.insertCell(pageBarTR.cells.length);afterCell.setAttribute('id','nextPageCell');if(pagingInfo.pagesCount==1){afterCell.innerHTML='<button onclick="GridTree.prototype._toAfterPage()" class="pagebarbtn disNextPage" title="��һҳ" id="_nextPageBtn"></button>';}
else{afterCell.innerHTML='<button onclick="GridTree.prototype._toAfterPage()" class="pagebarbtn nextPage" title="��һҳ" id="_nextPageBtn"></button>';}
var lastCell=pageBarTR.insertCell(pageBarTR.cells.length);lastCell.setAttribute('id','lastPageCell');if(pagingInfo.pagesCount==1){lastCell.innerHTML='<button onclick="GridTree.prototype._toLastPage()" class="pagebarbtn disLastPage" title="���һҳ" id="_lastPageBtn"></button>';}
else{lastCell.innerHTML='<button onclick="GridTree.prototype._toLastPage()" class="pagebarbtn lastPage" title="���һҳ" id="_lastPageBtn"></button>';}
var selectPageCell=pageBarTR.insertCell(pageBarTR.cells.length);selectPageCell.setAttribute('id','selectPageCell');selectPageCell.innerHTML=['<input type="text" style="width:30px" id="_pageNum" ','onkeypress="return event.keyCode>=48&&event.keyCode<=57||event.keyCode==46"','onpaste="return !clipboardData.getData(\'text\').match(/\D/)">','<button class="pagebarbtn jumpPage" onclick="GridTree.prototype._toPage(-1)" title="��ת" id="_toPageBtn"></button>'].join('');var selectPagingCell=pageBarTR.insertCell(pageBarTR.cells.length);selectPagingCell.innerHTML='ÿҳ<select onchange="GridTree.prototype._reMakeTable(this.value);" id="_changePageSizeSel"><option value="5">5</option><option value="10" selected>10</option><option value="20">20</option><option value="40">40</option><option value="100">100</option></select>��';var msgCell=pageBarTR.insertCell(pageBarTR.cells.length);msgCell.style.textAlign='right';msgCell.setAttribute('id','msgCell');msgCell.innerHTML=["��ǰ��",pagingInfo.currentPage,"ҳ/�ܹ�",pagingInfo.pagesCount,"ҳ"].join('');if(_style==1)
...
{if(pagingInfo.pagesCount!=0){if(_serverPagingMode=='client'){pagingInfo.pageSize=pageSize;pagingInfo.pagesCount=Math.ceil(pagingInfo.allCount/pagingInfo.pageSize*1.0);pagingInfo.currentPage=1;_$('msgCell').innerHTML="��ǰ��"+pagingInfo.currentPage+"ҳ/�ܹ�"
+pagingInfo.pagesCount+"ҳ";this._toPage(1);_$(tc.tableId).focus();}
...
GridTree.prototype._toPage=function(num,operCode){var pnum=_$('_pageNum'),mdiv=_$('_msgDiv'),mcel=_$('msgCell'),tb=_$(tc.tableId);if(num==-1){num=pnum.value;if(num==''||num>pagingInfo.pagesCount){console.log("����ҳ���������Ϊ�գ����������룡");pnum.focus();return;}else{operCode='refresh';}}
if(_serverPagingMode=='server'){GridTree.prototype._showMsg(true);jQuery.ajax({type:"POST",url:tc.dataUrl,data:'gtstart='+num+'&idColumn='+tc.idColumn+"&parentColumn="+tc.parentColumn+"&gtlimit="+pagingInfo.pageSize+"&gtoper="+operCode+"&gtcount="+pagingInfo.allCount,async:1,success:function(msg){var o=new Date();mdiv.innerText='���ڻ������...';GridTree.prototype._newPagingServerMakeTable(tb,msg);GridTree.prototype._showMsg(0);mcel.innerHTML=["��ǰ��",pagingInfo.currentPage,"ҳ/�ܹ�",pagingInfo.pagesCount,"ҳ"].join('');var gotime=new Date()-o;GridTree.prototype._wirteDebug('��ҳ��ʾǰ̨���ʱ��:'+gotime);}});}
else{pagingInfo.currentPage=num;var end=pagingInfo.currentPage*pagingInfo.pageSize>pagingInfo.allCount?pagingInfo.allCount:pagingInfo.currentPage*pagingInfo.pageSize;GridTree.prototype._showTable(document.getElementById(tc.tableId),(pagingInfo.currentPage-1)*pagingInfo.pageSize,end);mcel.innerHTML=["��ǰ��",pagingInfo.currentPage,"ҳ/�ܹ�",pagingInfo.pagesCount,"ҳ"].join('');if(tc.expandAll){GridTree.prototype.expandAll();}
...
if(isIE)
document.createStyleSheet(csspath);else{var headElement=document.getElementsByTagName("head")[0];var tempStyleElement=document.createElement('link');tempStyleElement.setAttribute("rel","stylesheet");tempStyleElement.setAttribute("type","text/css");tempStyleElement.setAttribute("href",csspath);}}
